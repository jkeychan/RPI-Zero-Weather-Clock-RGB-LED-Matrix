...
static void InterruptHandler(int signo) { interrupt_received = true; (void)signo; }
...
bool LoadConfig(const std::string& path, AppConfig& cfg)
{
    ...
    while (std::getline(in, line))
    {
        ...
        {
            if (key == "time_format") cfg.time_format = SafeInt(val, 24);
            else if (key == "temp_unit") cfg.temp_unit = val.empty() ? 'F' : static_cast<char>(toupper(val[0]));
            else if (key == "text_cycle_interval") cfg.text_cycle_interval = SafeInt(val, 10);
            else if (key == "AUTO_BRIGHTNESS_ADJUST") cfg.auto_brightness = (val == "true" || val == "True" || val == "1");
            else if (key == "BRIGHTNESS") cfg.brightness = SafeInt(val, 20);
            else if (key == "MANUAL_BRIGHTNESS") cfg.manual_brightness = SafeInt(val, 50);
            else if (key == "MIN_BRIGHTNESS") cfg.min_brightness = SafeInt(val, 20);
            else if (key == "MAX_BRIGHTNESS") cfg.max_brightness = SafeInt(val, 60);
            else if (key == "FRAME_INTERVAL_MS") cfg.frame_interval_ms = SafeInt(val, 60);
            else if (key == "BRIGHTNESS_UPDATE_SECONDS") cfg.brightness_update_seconds = SafeInt(val, 10);
            else if (key == "DYNAMIC_COLOR_INTERVAL_SECONDS") cfg.dynamic_color_interval_seconds = SafeInt(val, 1);
            else if (key == "LANGTONS_ANT_ENABLED") cfg.langtons_ant = (val == "true" || val == "True" || val == "1");
            else if (key == "FONT_PATH") cfg.font_path = val;
            else if (key == "LOG_LEVEL") cfg.log_level = val;
        }
        else if (section == "NTP")
        {
            if (key == "preferred_server") cfg.ntp_server = val;
        }
    }
    return true;
}
...
void WeatherThread(const AppConfig& cfg, WeatherState& state)
{
    ...
    if (!curl) return;
    std::string url = "https://api.openweathermap.org/data/2.5/weather?zip=" +
                      cfg.zip_code + "&appid=" + cfg.api_key + "&units=metric";
    std::string buffer;
    long backoff = 5;
...
static Color TempColor(int tempCelsius)
{
    ...
    return Color(static_cast<int>(r * 255),
...
int main(int argc, char** argv)
{
    ...
    Logger logger("/var/log/rgb/weather_clock.log",
                  Logger::ParseLevel(cfg.log_level));
    ...
    logger.Info("Matrix initialized: " + std::to_string(options.cols) + "x" +
                std::to_string(options.rows) + " slowdown=" +
                std::to_string(runtime.gpio_slowdown));
    ...
    // Wait up to 15s for the first weather fetch before starting the display loop
    {
        ...
        bool ok = weather_ready_cv.wait_for(lk, std::chrono::seconds(15),
                                             [] { return weather_fetched; });
        if (!ok) logger.Warning("Weather fetch timed out — starting with defaults");
    }
    ...
    while (!interrupt_received)
    {
        ...
        if (cfg.auto_brightness && (now_s - last_brightness >= cfg.brightness_update_seconds))
        {
            ...
            if (sr > 0 && ss > 0 && now_local >= sr && now_local <= ss)
            {
                ...
                double frac = (static_cast<double>(now_local) <= solar_noon)
                    ? (static_cast<double>(now_local - sr)) / (solar_noon - static_cast<double>(sr))
                    : (static_cast<double>(ss - now_local)) / (static_cast<double>(ss) - solar_noon);
        ...
        {
            rgb_matrix::DrawText(offscreen, font, scroll_x, 24,
                                 Color(255, 255, 255), weather_text.c_str());
            if (scroll_x + est < 0) scroll_x = offscreen->width();
            else --scroll_x;
        }
        else
        {
            rgb_matrix::DrawText(offscreen, font, 2, 24,
                                 Color(255, 255, 255), weather_text.c_str());
