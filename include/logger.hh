...
public:
    ...
    void Debug(const std::string& msg) { Write(Level::DEBUG, "DEBUG", msg); }
    void Info(const std::string& msg) { Write(Level::INFO, "INFO", msg); }
    void Warning(const std::string& msg) { Write(Level::WARNING, "WARNING", msg); }
    void Error(const std::string& msg) { Write(Level::ERROR, "ERROR", msg); }
...
private:
    ...
    void Rotate()
    {
        ...
        for (int i = kKeepFiles - 1; i > 0; --i) {
            ...
            std::error_code ec;
            if (fs::exists(older, ec)) fs::remove(older, ec);
            if (fs::exists(newer, ec)) fs::rename(newer, older, ec);
        }
    }
    ...
    void Write(Level level, const char* label, const std::string& msg)
    {
        ...
        namespace fs = std::filesystem;
        std::error_code ec;
        if (fs::exists(path_, ec) && fs::file_size(path_, ec) >= kRotateBytes)
        {
            Rotate();
        }
